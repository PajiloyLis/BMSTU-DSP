...
class PlotDrawer():
    ...
    @staticmethod
    def drawPlotThirdLab(xs, ys, figure: Figure, axes_names, title):
        ax = figure.axes[0]
        ax.clear()
        ax.plot(xs, ys, )
        ax.grid(True)
    ...
    @staticmethod
    def drawPlotFourthLab(xs, ys, figure: Figure, legend, axes_names, title):
        ax = figure.axes[0]
        ax.clear()
        ax.grid(True)
        ax.plot(xs[0], ys[0], 'b-', label=legend[0])
        ax.plot(xs[1], ys[1], 'g--', label=legend[1])
        ax.plot(xs[2], ys[2], 'r-.', label=legend[2])
        ax.set_title(title)
        ax.set_xlabel(axes_names[0])
        ax.set_ylabel(axes_names[1])
        ax.legend()